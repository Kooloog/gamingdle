...
function deleteCookie(name) {
    if (getCookie(name)) {
        document.cookie = name +'=; Path=/gamingdle/archive; Expires=Thu, 01 Jan 1970 00:00:01 GMT;';
    }
}
...
function showResults(attempts) {
    ...
    var message = 'Gamingdle (Archive) ' + currentPuzzle + ' - ' + (todayAttempts) + "/6<br><br>";
    ...
    box.style.animationTimingFunction = 'ease-out'
    box.style.animationFillMode = 'forwards';
    message += "\nhttps://kooloog.github.io/gamingdle/"
    copypaste = message;
}
...
fetch('answers.txt').then(response => response.text()).then(text => {
    ...
    document.getElementById("subtitle").innerHTML = "Currently playing Gamingdle #" + currentPuzzle;
});

document.getElementById("maingamingdle").onclick = function () {
    location.href = "https://kooloog.github.io/gamingdle/";
};
...
function changeFont() {
    ...
    if(fontEnable.checked) {
        ...
        document.getElementById("maintitle").style.fontFamily = "gaming";
        document.getElementById("subtitle").style.fontFamily = "gaming";
        document.getElementById("answerinfo").style.fontFamily = "gaming";

        for (var i = 0; i < 6; i++) {
            for (var j = 0; j < 5; j++) {
                var cell = document.getElementById("grid").rows[i].cells[j];
                cell.style.fontFamily = "gaming";
            }
        }

        for(var i = 0; i < 5; i++) {
            document.getElementById("example1").rows[0].cells[i].style.fontFamily = "gaming";
            document.getElementById("example2").rows[0].cells[i].style.fontFamily = "gaming";
            document.getElementById("example3").rows[0].cells[i].style.fontFamily = "gaming";
        }
